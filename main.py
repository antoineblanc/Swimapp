...
def analyse_one(path, name, job_id):
    ...
    try:
        ...
        ef = extract_frames(path, tmpdir, max(0,go_time-0.2), 3.5, 30, "entry")
        ...
        sf = extract_frames(path, tmpdir, surf_start, 9.0, 15, "surf")
        ...
        tf = extract_frames(path, tmpdir, max(0,approx-10), 20.0, 10, "turn")
        ...
        if wall_t:
            ...
            pf = extract_frames(path, tmpdir, wall_t, 3.0, 30, "po")
        ...
        ff = extract_frames(path, tmpdir, max(0,duration-8), 9.0, 15, "fin")
